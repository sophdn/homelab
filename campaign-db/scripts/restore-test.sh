...
VERIFY_TABLES="${CS_VERIFY_TABLES:-worlds entities accounts media_attachments}"
...
# --- The OTHER half: do the image bytes those rows point at still exist? ------
# The dump carries media_attachments/maps ROWS; the bytes live under
# UPLOADS_DIR and are in no dump. Restoring only the dump yields a database
# where every count matches and every image 404s. Row counts alone cannot see
# that, so this check is what stops the test passing on a half-restore.
#
# Paths come from the RESTORED database (what a restore would ask for) and are
# resolved under the live uploads root (what a restore would find). A path with
# no file is a FAILURE, not a warning: it is precisely the state a
# Postgres-only backup produces, and the one this pair exists to prevent.
UPLOADS_DIR="${CS_UPLOADS_DIR:-/var/lib/campaign-settings/uploads}"
if [ ! -d "$UPLOADS_DIR" ]; then
  log "VERIFY FAIL: uploads root $UPLOADS_DIR is absent - the media half is not there to restore"
  FAILED=1
else
  MEDIA_REFS="$(psql_q "$TARGET_DB" "SELECT file_path FROM media_attachments WHERE deleted_at IS NULL UNION ALL SELECT image_path FROM maps WHERE image_path IS NOT NULL AND deleted_at IS NULL;" 2>/dev/null || echo UNREADABLE)"
  if [ "$MEDIA_REFS" = "UNREADABLE" ]; then
    log "VERIFY FAIL: could not read media paths from the restored database - the check could not be made"
    FAILED=1
  else
    MEDIA_MISSING=0
    MEDIA_TOTAL=0
    while IFS= read -r rel; do
      ...
      if [ ! -f "$UPLOADS_DIR/$rel" ]; then
        log "VERIFY FAIL: referenced file missing under $UPLOADS_DIR -> $rel"
...
EOF
    if [ "$MEDIA_MISSING" -gt 0 ]; then
      log "VERIFY FAIL: $MEDIA_MISSING of $MEDIA_TOTAL referenced media files are missing"
      FAILED=1
    else
      log "verify ok: all $MEDIA_TOTAL referenced media files resolve under $UPLOADS_DIR"
    fi
  fi
fi

[ "$FAILED" -eq 0 ] || { log "FATAL: restore verification FAILED for $(basename "$NEWEST")"; exit 7; }
log "restore test PASSED for $(basename "$NEWEST") (rows + media bytes)"
