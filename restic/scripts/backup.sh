...
#
# /var/lib/campaign-settings/uploads is the OTHER HALF of the campaign backup.
# The pg_dump above carries the rows; the image bytes those rows point at live
# on the filesystem and are in no dump. Restoring only the dump gives a database
# where every row is present, every count matches, and every image 404s — a
# half-restore that looks complete, which is the worst shape a backup can fail
# in. It is captured AFTER the dump on purpose: a media row is inserted only
# once its bytes are on disk, so a file with no row is inert while a row with no
# file is a broken image. Added 2026-08-01 when uploads shipped (chain 451);
# UPLOADS_DIR is set in /etc/campaign-settings.env and this path must follow it.
...
restic backup \
  ...
  /var/lib/dm-toolkit \
  /var/lib/campaign-settings/uploads \
  /etc/act_runner \
