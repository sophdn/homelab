...
├── jellyfin/          # media server (host net :8096) + the media-ingest tool
...
| jellyfin | jellyfin | host:8096 | Media server. Ingest convention + tool: `jellyfin/MEDIA-INGEST.md` |
...
jellyfin/env/
  jellyfin.env         # gitignored — Jellyfin API key for the media-ingest scan trigger
  jellyfin.env.example
```

(The `jellyfin` **container** needs no env file — its `env/` holds the API key used
by `jellyfin/scripts/media_ingest.py`, not by the stack. `gitea-actions` carries no
secret in-repo — its registration token lives in the external `act_runner_data` volume.)
...
- `/var/lib/campaign-settings/uploads/` — the campaign's uploaded image bytes. This is the **other half** of the campaign backup and is not optional: the `pg_dump` above carries the `media_attachments` and `maps` rows, while the files those rows point at live here and are in no dump. Restoring only the dump yields a database where every row is present, every count matches, and every image 404s — a half-restore that looks complete. Added 2026-08-01 when uploads shipped; the path follows `UPLOADS_DIR` in `/etc/campaign-settings.env` and must be updated with it.

**Proven, not assumed.** `campaign-db-restore-test.timer` runs weekly (Sun 04:30, after the nightly backup): it restores the newest dump into a throwaway database and asserts both halves — core tables non-empty against production, AND every `media_attachments.file_path` / `maps.image_path` in the restored database resolving to a real file under `UPLOADS_DIR`. Row counts alone would pass with every image missing, which is exactly the failure a Postgres-only backup produces.
