...
# Episode tag: S01E01, S01E04E05, S01E01-E02-E03.
_EPISODE_TAG_RE = re.compile(r"[Ss](\d{1,2})((?:[-\s]?[Ee]\d{1,2})+)")
...
def parse_release_name(raw: str) -> ReleaseName:
...
def parse_episode_tag(filename: str) -> tuple[int, list[int]] | None:
    ...
    episodes = [int(number) for number in _EPISODE_NUM_RE.findall(match.group(2))]
    if not episodes:
        return None
...
def classify(source: Path, parsed: ReleaseName) -> str:
    ...
    for child in source.iterdir():
        if child.is_dir() and _SEASON_DIR_RE.match(child.name):
            return "show"
    tagged = sum(1 for path in video_files(source) if parse_episode_tag(path.name))
...
def plan_source(
    ...
    *,
    ...
    parsed = parse_release_name(source.name)
