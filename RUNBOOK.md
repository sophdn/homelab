...
`gitea`, `toolkit-server`, `jellyfin`, `gitea-actions` need no secret `.env` to
come up (gitea/toolkit config is inline + secret-free; the runner's token lives
in the `act_runner_data` volume — see § 4/§ 8). `jellyfin/env/jellyfin.env` is
optional and unrelated to the stack: it holds the API key that lets
`jellyfin/scripts/media_ingest.py` trigger a library scan (see
`jellyfin/MEDIA-INGEST.md`).
